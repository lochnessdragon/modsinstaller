...
def getUser(id):
	if isinstance(id, int):
		response = apiRequest(api_base + "author/" + str(id)).json()
	else:
		response = apiRequest(api_base + "author/search/" + id).json()
	return response.status_code, response.json()
...
if __name__ == '__main__':
	...
	for modid in modlist:
		...
		if status_code == 200:
			print("🟩  Found project: " + Fore.GREEN + mod['title'])
		else:
		...
		if(mc_version in mod['download']['versions']):
			# we can safely download the mod
			#print(mod['download'])
